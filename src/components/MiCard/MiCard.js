...
const PromptBtn = styled.a.attrs({
...
const Pager = styled.li`
  ...
  cursor: pointer;
  &:hover span{
    background-color:${props => props.active ? props.theme.white : props.theme.primary};
  }
`;
...
const PagerDot = styled.span`
    ...
    border: ${props => props.active ? `2px solid ${props.theme.primary}` : `2px solid ${props.theme.white}`};
    background-color: ${props => props.active ? props.theme.white : props.theme.mute};
    transition: all .5s ease;
`;
...
export default class MiCard extends Component {
    ...
    handleRight = evt => {
        this.setState({index: this.state.index + 1 > this.props.items.length ? this.props.items.length : ++this.state.index});
    };
    ...
    appendPrompt = (whichPrompt) => {
        return (
            <Item key='prompt'>
                ...
                <PromptBtn color={this.props.themeColor} href={whichPrompt.href}>{whichPrompt.to}</PromptBtn>
                <ItemFigure>
                    <ItemLink>
                        <img src={whichPrompt.url} width="160"
                             height="140"
                             alt={whichPrompt.to}/>
                    </ItemLink>
                </ItemFigure>
    ...
    generateList = () => {
        let whichPrompt = this.state.prompts[this.props.category];
        ...
        return (
            this.props.items && this.props.items
                ...
                .concat(this.appendPrompt(whichPrompt))
        );
    };
        ...
        return (
            <ThemeProvider theme={theme}>
                <Card color={this.props.themeColor}>
                    <Header color={this.props.themeColor}>{this.state.category[this.props.category]}</Header>
                    ...
                    <ArrowWrapper id="arrows">
                        <Arrow onClick={this.handleLeft} className="iconfont icon-back"/>
                        <Arrow onClick={this.handleRight} className="iconfont icon-more"/>
                    </ArrowWrapper>
                </Card>
            </ThemeProvider>
        );
